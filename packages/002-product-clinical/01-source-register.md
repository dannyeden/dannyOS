...
| SRC-003 | Eden Pharmacy formulary | Not received | Pharmacy | Missing | Medication, formulation, strength, package, and pharmacy SKU normalization |
...
| FACT-002 | SRC-002 | Constraint 2 | Care is lab-free | Product-to-Protocol Matrix; exact scope pending SRC-004 |
| FACT-003 | SRC-002 | Constraint 3 | Optimize is lab-gated | Product-to-Protocol Matrix; exact labs and timing pending SRC-004 |
...
dispensing and formulary facts; Clinical owns care policy; Daniel resolves product
and commercial direction; Compliance resolves regulated-policy interpretation.

## Confidentiality and repository handling

The bundle labels multiple files private/confidential and limits reuse to Beluga
service integration. Raw files remain outside Git. This repository records only
the minimum derived architecture, provenance, hashes, and summarized conflicts
needed to design HealthOS. Any future raw-source storage requires an explicit
Compliance decision and access-control design.
